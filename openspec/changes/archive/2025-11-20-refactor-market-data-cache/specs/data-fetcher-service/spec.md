# Capability: Data Fetcher Service
...
Data Fetcher Service is a background asyncio task that fetches market data from ThetaData MCP every 5 minutes during trading hours and maintains data freshness for watchlist symbols.
...
- **Depends on**: `market-data-storage` for inserting fetched data
- **Depends on**: ThetaData MCP server for market data
- **Used by**: `market-data-skill` for triggering lazy backfill
- **Modifies**: `runtime/main_loop.py` to add background task

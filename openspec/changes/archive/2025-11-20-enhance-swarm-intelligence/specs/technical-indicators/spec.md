# Spec: Technical Indicators Library

**Capability ID:** `technical-indicators`
**Status:** Draft
**Created:** 2025-11-20
...
**Depends On:**
- `market-data-storage`: Requires cached historical OHLCV bars

**Depended On By:**
- `mean-reversion-strategy`: Uses Bollinger Bands, RSI, support/resistance
- `breakout-strategy`: Uses ATR, Bollinger Bands, volume analysis
- `correlation-arbitrage-strategy`: Uses correlation calculations, z-scores
- All existing strategy templates (vol_sniper, trend_scout)

**Related Capabilities:**
- `market-data-skill`: Provides data input for indicator calculations
...
**Technology Choices:**
- NumPy for vectorized calculations (10-100x speedup vs pure Python)
- Avoid external dependencies (TA-Lib requires compilation)
- Use `@numba.jit` for complex indicators if needed (future optimization)

**Testing Requirements:**
- 100+ unit tests covering all indicators
- Edge case tests (NaN, empty arrays, insufficient data)
- Accuracy tests vs reference implementations (TA-Lib, known values)
- Performance benchmarks (10x speedup requirement)

**Data Format:**
Input: `List[Dict]` with keys `['open', 'high', 'low', 'close', 'volume', 'timestamp']`
Output: NumPy arrays or dicts of arrays (depending on indicator)

**Error Handling:**
- Invalid parameters: Raise `ValueError` with clear message
- Insufficient data: Return NaN array with warning log
- NaN input: Use `np.nan*` functions to skip NaN values

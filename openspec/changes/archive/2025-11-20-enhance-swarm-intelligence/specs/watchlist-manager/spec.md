# Spec: Automated Watchlist Manager

**Capability ID:** `watchlist-manager`
**Status:** Draft
**Created:** 2025-11-20
...
**Depends On:**
- `market-data-storage`: Requires trade history from trades.db
- `data-persistence`: Uses watchlist table for symbol tracking

**Depended On By:**
- Commander workflow: Uses updated watchlist for market snapshot
- All strategy templates: Analyze only watchlist symbols

**Related Capabilities:**
- `market-data-skill`: Provides performance data for scoring
...
- Sharpe Ratio: 40% (most important - risk-adjusted return)
- Win Rate: 30% (consistency)
- Avg P&L: 20% (absolute return)
- Trade Frequency: 10% (opportunity)

**Rotation Frequency:**
- Weekly evaluation (every Monday)
- Allows sufficient time to establish performance patterns
- Avoids daily churn

**Sector Classification:**
Use standard sector definitions:
- Technology (XLK): AAPL, NVDA, MSFT, GOOGL, etc.
- Finance (XLF): JPM, BAC, GS, etc.
- Energy (XLE): XOM, CVX, etc.
- Healthcare (XLV): UNH, JNJ, etc.
...
**Error Handling:**
- Insufficient trade history: Use longer lookback (180 days)
- No candidates available: Skip rotation
- Sector data unavailable: Use "UNKNOWN" sector (max 10% unknown)

**Database Schema:**
Add performance tracking columns to watchlist table:
```sql
ALTER TABLE watchlist ADD COLUMN sharpe_ratio REAL;
ALTER TABLE watchlist ADD COLUMN win_rate REAL;
ALTER TABLE watchlist ADD COLUMN avg_pnl REAL;
ALTER TABLE watchlist ADD COLUMN trade_count INTEGER;
ALTER TABLE watchlist ADD COLUMN last_evaluated TEXT;
```

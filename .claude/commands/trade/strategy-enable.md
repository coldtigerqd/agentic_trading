...
**用法**:
```
/trade:strategy-enable INSTANCE_ID
```

**示例**:
```
/trade:strategy-enable momentum_tech_short
/trade:strategy-enable volatility_breakout_all
```
...
from skills.strategy_manager import enable_strategy, get_strategy_config
...
if not instance_id or instance_id == "USER_INSTANCE_ID_HERE":
    print("错误：请提供策略实例ID")
    print()
    print("用法: /trade:strategy-enable INSTANCE_ID")
    print("查看可用策略: /trade:strategies")
    exit(1)

print("=" * 70)
print(f"启用策略实例: {instance_id}")
print("=" * 70)
print()
